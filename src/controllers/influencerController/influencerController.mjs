...
export const requestInfluencer = catchAsync(async (req, res, next) => {
  ...
  if (exists) {
    return next(
      new AppError("You have already requested to be an influencer", 409),
    );
  }
  ...
  if (!influencer) return next(new AppError("Request failed try again", 400));
  if (isAdmin) {
    await influencerStatusMail(`${firstName} ${lastName}`, email, status);
    return sendSuccess(
      res,
...
export const approveRejectInfluencer = catchAsync(async (req, res, next) => {
  ...
  return sendSuccess(
    res,
    "Influencer status updated successfully",
    {},
    200,
    true,
  );
